package fr.imie.skills.dto;

public class CompetenceDTO {

	private int idCompetence;
	private String libelleCompetence;
	private int competencePere;
	
	
	public int getIdCompetence() {
		return idCompetence;
	}
	public void setIdCompetence(int idCompetence) {
		this.idCompetence = idCompetence;
	}
	public String getLibelleCompetence() {
		return libelleCompetence;
	}
	public void setLibelleCompetence(String libelleCompetence) {
		this.libelleCompetence = libelleCompetence;
	}
	public int getCompetencePere() {
		return competencePere;
	}
	public void setCompetencePere(int competencePere) {
		this.competencePere = competencePere;
	}
	
	
	
}
